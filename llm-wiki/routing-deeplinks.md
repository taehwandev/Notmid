...
execution is reusable through the `:core:runtime` router package:
...
and host/base-path values to the reusable `:core:runtime` bundle. Core router impl
owns URI parsing, base-path stripping, deep-link matching, and route-event
handler dispatch. The `:core:runtime` runtime owns Compose state, pending request
